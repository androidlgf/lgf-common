...
public class FileUploadHelper {
    ...
    public String upload(@NonNull PostFormRequest postFormRequest) {
        if (TextUtils.isEmpty(postFormRequest.url) || postFormRequest.mListOfFileInfo.isEmpty()) {
            throw new IllegalArgumentException("you must make sure mPostFormRequest.url is not null or mFileInfos is not empty");
        }
//        postFormRequest.url = RequestConnect.parseUrl(postFormRequest.url);
        HttpURLConnection connection = createConnection(postFormRequest.url);
        if (connection == null) {
            return null;
        }
        addHeaders(connection, postFormRequest.headers);
        addFiles(connection, postFormRequest.mListOfFileInfo);
    ...
    public void uploadAsync(@NonNull final PostFormRequest postFormRequest,
        ...
        AsyncTaskHelper.doAsyncTask("upload",
                new AsyncTaskHelper.IDoAsyncTask<Void, String>() {
                ...
                new AsyncTaskHelper.IAsyncCallback<String>() {
                    @Override
